...
fn merge(config: &Config, input: &MergeInput) -> Result<Vec<u8>, HttpError> {
    ...
    let kind = if input.merge_base.is_some() {
        gix::object::Kind::Tree
    } else {
        gix::object::Kind::Commit
    };
    for hash in input.merge_base.iter().chain([&input.ours, &input.theirs]) {
        if !git_locator::import::commit(hash) {
            return Err(HttpError::new(
                400,
                "merge-tree requires full object hashes",
            ));
        }
        ...
        let header = repo
            .find_header(id)
            .map_err(|_| HttpError::new(400, format!("missing object {id}")))?;
        if header.kind() != kind {
            return Err(HttpError::new(400, format!("{id} is not a {kind}")));
        }
    ...
    if input.merge_base.is_some() {
        command.arg(format!("--merge-base={}", trees.remove(0)));
    }
    let result = command.args(&trees).output()?;
...
mod tests {
    ...
    #[test]
    fn source_merge_and_explicit_tree_merge_share_the_same_operation() {
        let store = Store::new();
        let base = store.tree(&[("file", "original\n")]);
        let ours = store.tree(&[("file", "original\n"), ("ours", "new\n")]);
        let theirs = store.tree(&[("file", "changed\n")]);
        let commit = |tree: &str, parents: &[&str]| {
        ...
        let base_commit = commit(&base, &[]);
        let ours_commit = commit(&ours, &[&base_commit]);
        let theirs_commit = commit(&theirs, &[&base_commit]);
        ...
        .unwrap();
        assert_eq!(native, store.merge(&base, &ours, &theirs));
    }
