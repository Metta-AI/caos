...
pub const TURNS_DIR: &str = ".caos/turns";
pub const ACTIVE_TURN: &str = ".caos/turns/active";
pub const CALLS_DIR: &str = ".caos/calls";
pub const TASK_COMPUTATIONS_DIR: &str = ".caos/tasks/computations";
pub const TASK_CONVERSATIONS_DIR: &str = ".caos/tasks/conversations";
...
// Historical v3 paths are accepted on read and exact replay only.
pub fn legacy_path(path: &str) -> Option<String> {
    for (current, old) in [
        (TURNS_DIR, ".caos/requests"),
        (CALLS_DIR, ".caos/tools"),
        (TASK_COMPUTATIONS_DIR, ".caos/async"),
        (TASK_CONVERSATIONS_DIR, ".caos/subagents"),
    ] {
...
pub fn current_path(path: &str) -> String {
    for (current, old) in [
        (TURNS_DIR, ".caos/requests"),
        (CALLS_DIR, ".caos/tools"),
        (TASK_COMPUTATIONS_DIR, ".caos/async"),
...
mod tests {
    ...
    fn builders_have_exact_layouts() {
        ...
        assert_eq!(turn_record_path("r"), ".caos/turns/r.json");
        assert_eq!(ACTIVE_TURN, ".caos/turns/active");
        assert_eq!(
            call_record_path("r", 7, "toolu_01AB"),
            ".caos/calls/r/0007/toolu_01AB.json"
        );
        assert_eq!(
            call_payload_dir("r", 7, "toolu_01AB"),
            ".caos/calls/r/0007/toolu_01AB"
        );
        assert_eq!(
            call_record_path("r", 7, "weird id!"),
            ".caos/calls/r/0007/776569726420696421.json"
        );
        assert_eq!(
            async_record_path("task"),
            ".caos/tasks/computations/task.json"
        );
        assert_eq!(
            subagent_record_path("child"),
            ".caos/tasks/conversations/child.json"
        );
        assert_eq!(
