...
impl Transition {
    pub fn stack_base(name: String, config: super::WorkspaceConfig) -> Self {
        ...
        if let Some(super::WorkspaceBase::Branch {
            repository: Some(repository),
            name: branch,
            commit,
        }) = config.upstream
        {
            let base = super::workspaces::BaseUrl { repository, branch };
...
fn apply_files(
    ...
    for (relative, value) in files {
        paths::validate_workspace_name(relative)?;
