...
fn execution_roundtrip_leaves_no_lifecycle_files() {
    ...
    assert_eq!(
        ...
        fork_view.workspace("main").unwrap().unwrap().commit
    );
    assert_ne!(
        view.reference_start("main").unwrap(),
        view.workspace("main").unwrap().unwrap().commit
    );
...
fn code_paths_are_content_and_sealing_preserves_code_history() {
    ...
    assert_eq!(
        view.workspace_names().unwrap(),
        ["feature/00-base", "feature/01-change"]
    );
    assert_eq!(
        view.workspace("feature/01-change").unwrap().unwrap().commit,
        w
    );
    assert_eq!(
    ...
    assert_eq!(
        view.workspace_config("feature/01-change")
            .unwrap()
            .publication
            .unwrap()
            .branch,
