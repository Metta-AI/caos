...
pub(crate) fn send(
    ...
    let (server, headers) = prepare(source, token_file)?;
    post(&server, endpoint, body, &headers)
}

type Headers = Vec<(&'static str, String)>;

pub(crate) fn prepare(source: &str, token_file: Option<&str>) -> Result<(String, Headers), String> {
