...
fn render_footer(app: &App, frame: &mut Frame<'_>, area: Rect) {
    let line = match app.notice() {
        Some(notice) => Line::styled(format!(" {notice}"), Style::default().fg(Color::Red)),
        None => Line::styled(
            format!(
                " Enter send  Up/Down scroll  Ctrl+L refresh  Esc quit  [{}]",
                app.conversation()
            ),
            Style::default().fg(Color::DarkGray),
        ),
...
fn status_color(status: &str) -> Color {
    match status {
        ...
        "queued" | "running" | "working" | "submitting" => Color::Yellow,
        _ => Color::Green,
    }
}
