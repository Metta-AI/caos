...
//! under the conversation directory (`refs/caos/conversations/<name>/…`)
//! alongside the head (`from-user`), so a conversation's refs all sit together:
//!
//! * `refs/caos/conversations/<name>/from-agent` — after each step commit, the
...
/// Point the conversation's `from-agent` ref at `new_hash`, warning (never
/// failing) on any error.
pub fn push(conversation: &str, new_hash: &str) {
    let refname = format!("refs/caos/conversations/{conversation}/from-agent");
