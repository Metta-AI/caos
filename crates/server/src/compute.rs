//! Compute: the `/run` and `/run-async` pipeline.
//!
...
pub(crate) fn run(
    ...
    // For a blocking caller this ref is convenient; for `/run-async` it is the
    // only durable way to discover the eventual result from Q. Therefore a run
    // is not successful until publication succeeds. Repeating exact Q is safe:
    // the compute result is cached, and the retry comes straight back here.
    pin_result(config, &arg_tree, &result)?;
    Ok(format!("{result}\n").into_bytes())
}

/// `GET /run-async?req=<argTreeHash>` — ensure the top-level request is
/// progressing after the HTTP caller goes away, and return its identity as
/// `"request <argTreeHash>"` without waiting for the result.
///
/// The request object is checked synchronously, so success means there is a
/// runnable request to hand off rather than merely that a thread was spawned.
/// Repeated calls join through the ordinary compute single-flight, exactly as
/// concurrent blocking `/run` calls do.
pub(crate) fn run_async(
    config: Arc<Config>,
    query: String,
    secrets_header: String,
) -> Result<Vec<u8>, HttpError> {
    let arg_tree = parse_arg_tree(&query)?;

    // Fail while the caller can still hear us if Q is absent or malformed.
    // `run` reads it again after handoff because the normal pipeline owns all
    // execution validation; this first read is deliberately only admission.
    let (image, _) = read_arg_tree(&config, &arg_tree)?;
    if image.is_empty() {
        return Err(HttpError::new(400, "request has empty image"));
    }

    let request_id = arg_tree.clone();
    let thread_name = format!("caos-run-async-{}", &arg_tree[..arg_tree.len().min(12)]);
    std::thread::Builder::new()
        .name(thread_name)
        .spawn(move || {
            if let Err(err) = run(&config, &query, &secrets_header) {
                eprintln!("async run failed: arg_tree={request_id}: {}", err.message());
            }
        })
        .map_err(|err| HttpError::new(500, format!("starting async run for {arg_tree}: {err}")))?;

    Ok(format!("request {arg_tree}\n").into_bytes())
}

/// Parse and validate the request identity shared by both compute surfaces.
...
/// for `/run-async`, whose caller receives Q rather than the eventual result.
