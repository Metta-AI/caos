...
use super::workspace::{
    ...
    load_conversation_workspace, load_repo_agent_config, local_default_branch_tip,
    prepare_publish_workspace, publish_conversation_branch, publish_conversation_pr,
    remote_base_is_ancestor, remote_default_branch, PublishedConversationSource,
};
...
impl App {
    ...
    fn publish_selected(&mut self) {
            ...
            .is_none_or(|diff| diff.patch.is_empty())
        {
            ...
            std::thread::spawn(move || {
                let result = (|| {
                    let agent_config = load_repo_agent_config(&repo_dir)?;
                    ...
                    let message = publish_turn_message(
                        &target,
                        base_is_ancestor,
                        agent_config.pr_publish_instructions.as_deref(),
                    );
                    let outcome = run_chat_turn(
                        &transport,
                        &options,
...
fn publish_turn_message(
    target: &str,
    base_is_ancestor: bool,
    pr_publish_instructions: Option<&str>,
) -> String {
    let mut message = if base_is_ancestor {
        format!(
        ...
        format!(
            "Prepare this conversation for publication. First call the existing `merge` tool \
             ...
             `.caos/conflicts`, and finish only when the workspace is ready to publish."
        )
    };
    if let Some(instructions) = pr_publish_instructions {
        message.push_str("\n\nRepository publication instructions:\n");
...
mod tests {
    ...
    #[test]
    fn publish_prompt_only_requests_relevant_merge_and_repository_instructions() {
        let already_merged = publish_turn_message("abc123", true, None);
        assert!(already_merged.contains("do not call `merge`"));
        assert!(!already_merged.contains("First call the existing `merge` tool"));
        assert!(!already_merged.contains("Build and test"));
        assert!(!already_merged.contains("Repository publication instructions"));

        let needs_merge =
            publish_turn_message("abc123", false, Some("Build and test the final workspace."));
        assert!(needs_merge.contains("First call the existing `merge` tool"));
        assert!(needs_merge.contains("`theirs` exactly `abc123`"));
        assert!(needs_merge.ends_with(
            "Repository publication instructions:\nBuild and test the final workspace."
        ));
    }
