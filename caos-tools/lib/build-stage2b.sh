...
# The shared cargo bake the root flake imports (std/cargo/bake.nix,
...
caos get /cas/args/workspace/std/cargo
mkdir -p /tmp/bake-ws/std/cargo
ln -s /cas/args/workspace/std/cargo/bake.nix /tmp/bake-ws/std/cargo/bake.nix
...
bake=$(caos curry "docker://$(cat /cas/args/children/nixbuilder)" -- \
  "--worker1:@=$LIB/bake.sh")
stage2c=$(caos curry /cas/std/bash -- "--worker1:@=$LIB/build-stage2c.sh" \
