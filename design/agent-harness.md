...
- **Compute tools** (bash, build, test, search): run-then sub-runs. Input
  ...
  enforced: workers currently sit on the compute network — a per-image egress
  fence is future work.)

The built-in file, bash, and grep tools remain bounded or decomposed. A custom
docs-driven tool receives the complete workspace tree and chooses what to
materialize inside its remote, ephemeral image; it still never checks a
conversation out into the user's local working tree.
...
- **docs-driven external tools** (implemented — `worker-llm-step/src/bundle.rs`):
  a bundle tree has one directory per model-facing tool, containing only
  `docs` and `image` blobs. The provider-required input schema is one generic,
  permissive object generated by llm-step; the opaque call reaches the worker
  as `call.json` beside the workspace. The fixed result convention is a text
  `result`, optional `is_error`, and optional updated `workspace`. `--tools`
  selects this tree for one client invocation; without it the existing tool
  registry is unchanged. Rust, Python, or other workers can implement the same
  protocol in different images without a harness change.
...
binary curried onto `/cas/std/runner` — the stub tests' path. `--tools` binds a
tracked docs-driven tool-set tree into llm-step for that invocation. Entries
carry only a doc string and an image ref, not typed per-tool input/output
schemas. With no `--tools`, the existing built-in and project tool registry is
unchanged.
