...
CAOS publishes exact code commits from its server. The GitHub worker manages PR metadata. See [agent-stacks.md](agent-stacks.md) for stack registration, restacking, conflict resolution and submission.
...
| `rewrite` | Optional boolean, default false. Permit a non-fast-forward update while still requiring the exact expected remote head. |
...
For a registered stack, submit_stack performs branch publication, PR lookup/creation, base correction and linking in order. It uses the same invocation claim for the entire submission and returns
partial receipts on failure. Existing PR titles and bodies are preserved.
...
Submission pushes source commits through the server, finds PRs by repository and branch, then creates or appends GitHub stack membership through gh api. Existing unmerged members must be an ordered
prefix; already-merged entries remain in GitHub's history. Removing or reordering members is a separate action.

The pinned gh-stack v0.1.1 link command cannot extend a stack containing merged PRs: it requires those entries in the input but rejects them as merged. The worker therefore uses the same [Stacks
API](https://github.com/github/gh-stack/blob/v0.1.1/internal/github/github.go) directly: list stacks, create one with ordered PR numbers, or append new numbers to an existing stack.

The general github tool still provides gh stack commands. CAOS does not use gh stack rebase/submit/sync; their local branch and worktree model does not match source gitlinks. No source checkout is
needed for the metadata calls above.
