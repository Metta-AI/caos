...
# How many nodes under a request were REUSED rather than performed by it — the
# trace's own answer (SPEC, "Tracing"), read after the run from the completed
# view. `prepare-request` forms and pushes the very ArgTree `run` will use, so
# the hash is known before the work starts and without running anything.
reused() { # <request hash> <name for the saved json>
  "$CAOS_CLI" status --all "$1" > "$2.status" 2>/dev/null \
    || fail "reading the trace for $1"
  jq '[.. | objects | select(.reused == true)] | length' "$2.status"
}

echo "== per-crate (mode=all): cold, then a one-crate edit ==" >&2
cold_req=$("$CAOS_CLI" prepare-request --base:@=DEEP-DEPS/cargo \
  --tree:@=ws --cmd=check --mode=all "--target=$tgt")
t2=$(ms)
"$CAOS_CLI" run r2 --base:@=DEEP-DEPS/cargo \
  --tree:@=ws --cmd=check --mode=all "--target=$tgt"
t3=$(ms)
[ "$(cat r2/exit)" = "0" ] || fail "mode=all check failed: $(tail -c 2000 r2/stderr)"
cold=$((t3 - t2))
cold_hits=$(reused "$cold_req" cold)
echo "  ok: per-crate check clean (${cold}ms cold, ${cold_hits} jobs reused)" >&2
...
# as a single-flight WAITER (which performs no work of its own and records
# nothing) or, if the first finished first, as a reuse of a record that ended
# before this parent started, is pure timing — so a genuinely cold run
# legitimately shows a small, jittery number of intra-run dedup reuses.
# Asserting ==0 was the flake. The real question — "did the salt take?" — is
# answered comparatively against the edit run below, where it has meaning and
# no race.
...
# asserts — structurally, out of the trace records, not by stopwatch.
#
...
# working perfectly). A count of reused jobs cannot be broken by making things
# faster.
#
# worker-runner is a LEAF — nothing depends on it — so every other member's job
# should be reused. The threshold is deliberately loose: the point is "most of
# the workspace was reused", not an exact count that would need editing whenever
# a crate is added.
echo "// tripwire edit" >> ws/crates/worker-runner/src/main.rs
commit "edit one crate"
edit_req=$("$CAOS_CLI" prepare-request --base:@=DEEP-DEPS/cargo \
  --tree:@=ws --cmd=check --mode=all "--target=$tgt")
t4=$(ms)
"$CAOS_CLI" run r3 --base:@=DEEP-DEPS/cargo \
  --tree:@=ws --cmd=check --mode=all "--target=$tgt"
t5=$(ms)
[ "$(cat r3/exit)" = "0" ] || fail "edited mode=all check failed: $(tail -c 2000 r3/stderr)"
edit_hits=$(reused "$edit_req" edit)
echo "  ok: one-crate edit reused ${edit_hits} jobs (cold reused ${cold_hits})" >&2
...
# the handful of intra-run diamond-dedup reuses, well below edit_hits. So a cold
# run with FEWER reuses than the reuse run is the honest signal, and it cannot be
# broken by the single-flight-vs-cache timing that broke the old ==0 assertion.
[ "$cold_hits" -lt "$edit_hits" ] \
  || fail "the cold run reused ${cold_hits} jobs vs ${edit_hits} in the edit run — the salt did not take, so nothing here is measuring what it claims"
[ "$edit_hits" -ge 8 ] \
  || fail "one-crate edit reused only ${edit_hits} jobs — per-crate caching regressed.
  Editing the leaf crate worker-runner should leave every other member's compile
  reused; the cold run before it reused ${cold_hits} by construction."
edit=$((t5 - t4))
