#!/bin/bash
# Repository writers through the real agent handler and server object APIs.
...
# The conversation helper fetches configuration before publishing its ref.
...
stage "public repository tools with the evaluated worker pinned"
...
for binding in rebase add-layer; do
  caos get "/cas/args/$binding-expr"
  expression=$(cat "/cas/args/$binding-expr")
  pinned=${expression/"$base_binding"/"--base:hash=$stack_worker"}
  [ "$pinned" != "$expression" ] || fail "$binding wrapper lacks the shared worker binding"
  case "$binding" in rebase) tool_name=git-rebase-i ;; add-layer) tool_name=git-add-layer ;; esac
  mkdir -p "/tmp/repository-tools/$tool_name"
  printf '%s\n' "$pinned" > "/tmp/repository-tools/$tool_name/.caos-expr"
done
tools_tree=$(publish_tree /tmp/repository-tools /cas/repository-tools "repository tools")

plan=$(printf 'onto=%s\npick=%s..%s\namend=first feature\nbranch=00-feature\npick=%s\namend=second feature\nbranch=01-second\n' "$h" "$a" "$b" "$c")
setup_cmd=$(cat <<EOF
mkdir -p feature/rebase promotion
...
mkdir -p /tmp/stub
jq -n --arg cmd "$setup_cmd" --arg plan "$plan" '{content:[
...
jq -n --arg message "$promotion_message" '{content:[
 {type:"tool_use",id:"help",name:"tool_help",input:{path:"tools/git-rebase-i"}},
 {type:"tool_use",id:"promote",name:"run_tool",input:{path:"tools/git-add-layer",arguments:{stack:"promotion",name:"promoted",author:"Promoter <test@caos> 1700000001 +0000",committer:"Promoter <test@caos> 1700000001 +0000",message:$message}}},
 {type:"tool_use",id:"start",name:"run_tool",input:{path:"tools/git-rebase-i",arguments:{stack:"feature",plan:"feature/rebase/plan"}}}
],stop_reason:"tool_use"}' > /tmp/stub/response-2.json
cat > /tmp/stub/response-3.json <<'JSON'
{"content":[{"type":"tool_use","id":"report","name":"read","input":{"file-path":"feature/rebase/conflicts"}},{"type":"tool_use","id":"draft","name":"read","input":{"file-path":"feature/rebase/work/file"}},{"type":"tool_use","id":"resolve","name":"write","input":{"file-path":"feature/rebase/work/file","content":"resolved\n"}}],"stop_reason":"tool_use"}
JSON
cat > /tmp/stub/response-4.json <<'JSON'
{"content":[{"type":"tool_use","id":"continue","name":"run_tool","input":{"path":"tools/git-rebase-i","arguments":{"stack":"feature","action":"continue"}}}],"stop_reason":"tool_use"}
JSON
printf '%s\n' '{"content":[{"type":"text","text":"done"}],"stop_reason":"end_turn"}' > /tmp/stub/response-5.json
start_stub /tmp/stub
new_llm_conversation llm-rebase "$STUB_PORT" - "Use the repository tools to promote work and replay the supplied stack."
dispatch_turn "Promote the work and restack the feature. ($SALT)"
wait_turn 600

stage "promotion squashes work history and starts the next work layer"
...
stage "finished replay retains source changes and intentional history"
...
[ "$(git show -s --format=%B "$first")" = 'first feature' ] || fail "wrong first amended message"
[ "$(git show -s --format=%B "$second")" = 'second feature' ] || fail "wrong second amended message"
...
jq -e '.tools | any(.name == "run_tool") and all(.name != "git-rebase-i")' /tmp/stub/request-1.json >/dev/null || fail "stack writers must be repository tools"
$TOOL tools --repo /tmp/repo --head "$head" --request "$request" > /tmp/rebase-tools.jsonl
jq -s -e 'all(.[]; .status == "complete")' /tmp/rebase-tools.jsonl >/dev/null || fail "a stack tool call failed"
for response in /tmp/stub/request-{2,3,4,5}.json; do
