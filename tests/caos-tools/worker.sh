...
# as the parameters), addressed by PATH. Both calls evaluate ancestors to find
# the definition. `tool_help` then READS its expression, so describing a
# compiled tool never builds it, and `run_tool` asks the server to EVALUATE it,
# then curries the model's
