#!/bin/bash
# Suite stage 4 (the `then` of the std warm-up): --map is the per-test worker
# stage 3 curried over the test stack image (design/test-stack-image.md).
# Select the tests and map-then over them — one stack per test, each carrying
...
# The map worker is the test stack image curried with the per-test runner —
# curried in stage 3 and handed here as a REF STRING, because currying over a
# curried arg path (`caos curry /cas/args/image`) curries the arg NODE and the
# worker then inherits this job's own bindings. That cost a debugging round:
# map jobs arrived with args `image in worker1`, unable to read /cas/args/in
# because the image's root grant was not in play either.
map=$(cat /cas/args/map)
