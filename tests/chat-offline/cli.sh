...
echo "== accepted input survives request-preparation failure ==" >&2
...
queued_tip=$(remote_tip "$queued_ref") \
  || fail "accepted message was lost when request preparation failed"
git fetch -q caos "$queued_tip"
queued_event=$(git show -s --format=%B "$queued_tip" | tr -d '\n')
[[ "$queued_event" == *'"author":"user","content":"hello"'* ]] \
  || fail "queued head does not contain the accepted user message"
[[ "$queued_event" == *'"status":"queued"'* ]] \
  || fail "request-preparation failure did not leave a queued turn"
if [[ "$queued_event" == *'"request"'* ]]; then
  fail "client recorded a request it did not prepare"
fi
...
# Choosing the newest chat reads only tip metadata first. A completely fresh
# client must still fetch the selected event's history/tree closure before
# replaying it; the metadata read alone is not a usable local snapshot.
...
(
  cd fresh-reader
  "$CAOS_CLI" talk --log > ../fresh-reader.log
)
grep -qF "tester: fresh start" fresh-reader.log \
  || fail "fresh client did not select/replay the newest conversation"
