...
identify=$("$CAOS_CLI" curry DEEP-DEPS/bash -- --worker1:@=test/identify.sh)
request=$("$CAOS_CLI" run "$identify" -- --tag=exact-request)
...
driver=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
  --worker1:@=test/driver.sh --request="$request")
actual=$("$CAOS_CLI" run "$driver" --)
...
callback=$("$CAOS_CLI" curry DEEP-DEPS/bash -- --worker1:@=test/callback.sh)
driver_then=$("$CAOS_CLI" curry "$driver" -- --then-img="$callback")
actual=$("$CAOS_CLI" run "$driver_then" --)
...
checks=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
  --worker1:@=test/checks.sh --request="$request")
actual=$("$CAOS_CLI" run "$checks" --)
...
boom=$("$CAOS_CLI" curry DEEP-DEPS/bash -- --worker1:@=test/boom.sh)
if "$CAOS_CLI" run "$boom" -- --tag=exact-failure 2>boom.err; then
...
failed_driver=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
  --worker1:@=test/driver.sh --request="$failed_request")
if "$CAOS_CLI" run "$failed_driver" -- 2>uncaught.err; then
...
caught_driver=$("$CAOS_CLI" curry "$failed_driver" -- \
  --then-img="$callback" --catch=1)
actual=$("$CAOS_CLI" run "$caught_driver" --)
