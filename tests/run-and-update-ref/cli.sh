...
[ "${success_tasks[0]}" = "$success_task" ] \
  || fail "status directory is not returned Q: ${success_tasks[0]} != $success_task"
...
git show -s --format=%B "$success_head" \
  | grep -qF '{"v":2}' \
  || fail "complete event message is not the minimal v2 event"
