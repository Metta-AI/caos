...
double=$("$CAOS_CLI" curry DEEP-DEPS/bash -- --worker1:@=test/double.sh)
combine=$("$CAOS_CLI" curry DEEP-DEPS/bash -- --worker1:@=test/combine.sh)

echo "== run with no then: a plain tail call to run ==" >&2
tail_driver=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
...
both_driver=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
...
# combine sees --result.
outer=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
  --worker1:@=test/outer.sh --inner-img="$double")
nested_driver=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
...
ok=$("$CAOS_CLI" run DEEP-DEPS/bash -- --worker1:@=test/checks.sh --in:@=in.txt)
...
catcher=$("$CAOS_CLI" curry DEEP-DEPS/bash -- --worker1:@=test/catcher.sh)
uncaught_driver=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
...
caught_driver=$("$CAOS_CLI" curry DEEP-DEPS/bash -- \
...
cyc=$("$CAOS_CLI" curry DEEP-DEPS/bash -- --worker1:@=test/cycle.sh)
