...
t0=$(ms)
"$CAOS_CLI" run DEEP-DEPS/cargo r1 -- --tree:@=test/ws --cmd=check --mode=all "--target=$tgt"
t1=$(ms)
...
"$CAOS_CLI" run DEEP-DEPS/cargo r2 -- --tree:@=test/ws --cmd=test --mode=all "--target=$tgt"
...
commit "break a"
"$CAOS_CLI" run DEEP-DEPS/cargo r3 -- --tree:@=test/ws --cmd=check --mode=all "--target=$tgt"
...
commit "fix a"
"$CAOS_CLI" run DEEP-DEPS/cargo r4 -- --tree:@=test/ws --cmd=check --mode=all "--target=$tgt"
[ "$(cat r4/exit)" = "0" ] || fail "fixed check failed: $(cat r4/stderr)"
sed -i 's/b says/b announces/' test/ws/b/src/main.rs
commit "edit b"
t2=$(ms)
"$CAOS_CLI" run DEEP-DEPS/cargo r5 -- --tree:@=test/ws --cmd=check --mode=all "--target=$tgt"
t3=$(ms)
...
t4=$(ms)
"$CAOS_CLI" run DEEP-DEPS/cargo r6 -- --tree:@=test/ws --cmd=check --mode=all "--target=$tgt"
t5=$(ms)
