...
- For a PR, finish preparing and checking the changes, then give /pr <conversation/gitlink> <base-remote-branch> [remote-URL], e.g. /pr feature/01-parser main. Omit the URL when matching import provenance identifies it; otherwise supply a known URL or ask only for what is missing. Use a known base branch (import metadata may provide the default), never guess one. The command loads a preview; Enter confirms pushing and opening or updating that PR. For a stack, suggest one command per boundary in order, e.g. /pr feature/02-errors feature/01-parser after the first PR is published. Each command publishes exactly its named snapshot; sibling names do not choose the PR base. /publish-branch <conversation/gitlink> [remote-URL] pushes a branch without creating a PR. The source and base must share Git history, but the source need not include the latest base tip. If the base branch is missing, correct the command or publish the preceding PR first. Do not request publication settings merely to prepare changes. Do not suggest Ctrl+P.
