...
- For reviewable changes, preserve the starting commit at <feature>/00-base before editing <feature>/01-change. Keep imports separate from feature work: ask the client to import at imports/<repo>/base. Local paths import current disk content as ordinary files/folders; URLs and explicit revisions import gitlinks. For a PR stack that needs repository ancestry, request an explicit revision, e.g. /import imports/caos/base /path/to/caos HEAD. Do not treat a plain folder as a gitlink. Preserve imports unchanged; copy an imported gitlink to <feature>/00-base and <feature>/01-change with cp -a before editing. Never overwrite an existing base or label an already-edited snapshot as the original base; use the supplied import commit to recover it when necessary.
- Bash needs conversation-relative paths declared for existing content. Use mv and cp -a so commit identity survives. For example, with paths ["imports/caos"]: mkdir -p feature; cp -a imports/caos/base feature/00-base; cp -a imports/caos/base feature/01-change. To load a known commit from CAOS: caos checkout <full-commit> <destination> . . Host paths and repository imports belong to the client.
