...
- To push a registered stack, use push_stack with path and an HTTPS repository URL. It pushes the source gitlink paths as branch names and creates no PRs or GitHub stack membership. Use rewrite=true only for intentional rebased-history updates. After uncertainty inspect the remote before a new call.
...
- Push stack layers bottom to top with push_stack. Each upper source commit must include the exact published lower commit. The tool pins commits and remote-head leases, records branch receipts, and stops after a failure. Never roll back successful pushes after a later branch fails.
- When a lower layer changes, restack the registered stack, test, and push again. Rebased publication requires explicit rewrite=true and retains an exact remote-head lease. Pushing a stack does not create PRs or GitHub stack membership.
