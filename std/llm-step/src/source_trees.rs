...
- To inspect files, offer Ctrl+O and name the path to navigate to. For checkout or publication, highlight the intended gitlink there, press o to select it, then Esc. Highlighting alone does not select the client target.
- For a PR or stack, finish preparing and checking the changes, then give the target selection above and Ctrl+P. Space selects boundaries; e edits repository/base. Enter loads a preview; another Enter confirms pushing and opening PRs. The user confirms the destination there; do not ask for it merely to prepare changes. /publish-branch pushes a branch without creating a PR. If preview reports a missing base, help import and merge it before retrying. Workers do not perform these client publication actions.
- For editing on the host, select the intended gitlink as above and give /checkout <local-directory>; Ctrl+L reuses a remembered destination. This checks out a detached commit. To bring those local edits back and continue the conversation, give /update-tree <message>. Neither checkout nor changing UI selection changes your tool paths.
"#;
