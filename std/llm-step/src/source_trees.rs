...
Stack workflow:
- Register an ordered set of existing source gitlinks with stack(action="create", path="feature", base="00-base", layers=["01-core","02-ui"]). Keep the base as an imported snapshot.
- After a lower-layer edit or an import of updated origin/main, use stack(action="rebase", path="feature", onto="<imported gitlink or hash>"). Use method="merge" for histories containing merge commits.
- A stack conflict leaves the source pointers unchanged. Read feature/restack/operation.json and edit/test feature/restack/work. Then stack(action="continue", path="feature", resolved=true) explicitly asserts that ALL reported conflicts are resolved, including structural ones. Do not create .caos/conflicts. Abort leaves sources unchanged.
- To publish or resubmit, use submit_stack with path, repository owner/repo and base_branch. New PR descriptions can be supplied in layer order. Existing PRs are discovered by branch, not remembered numbers. Use rewrite=true only for intentional rebased-history updates. After uncertainty inspect the remote before a new submission.
...
- For legacy non-stack merge operations, resolve source-tree conflicts by editing affected files and clearing their ledger entries. Saving an edited source tree removes an empty .caos/conflicts ledger and prunes its .caos directory if empty. Unresolved entries and other metadata stay intact. Do not recreate a removed ledger to register resolution. This source-tree ledger is distinct from protected conversation-root .caos.
...
- When publication is requested, use publish_source with an explicit gitlink, HTTPS repository and branch. Inspect the complete PR diff, incorporate the chosen base and prior stack boundary, resolve conflicts and run checks first. Use submit_stack for registered stacks and github for individual PR metadata. No local branch database is needed. Import provenance can identify the repository and default base; ask only when ambiguous.
...
- For origin/main, read the selected source's import provenance, use that repository URL and revision main with import_source, then use stack rebase for a registered stack, or merge it into an individual source tree. If provenance is ambiguous, use an explicit repository. Remote names are not local ref snapshots.

- Publish stack layers bottom to top. Each upper source commit must include the exact published lower commit. Find the PR by repository and head branch before creating one; name the base branch explicitly, preserve existing titles and bodies, and create ready-for-review PRs unless drafts were requested. Use submit_stack to link registered layers through GitHub's stack API, preserving already-merged members; verify bases and membership afterwards. If native stack linking is unavailable, retain the valid chained PRs and explain the limitation. Never roll back successful pushes after a later step fails.
