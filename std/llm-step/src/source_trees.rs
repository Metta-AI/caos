...
- For reviewable changes, preserve the starting commit at <feature>/00-base before editing <feature>/01-change. Keep imports separate from feature work: ask the client to import at imports/<repo>/base. Local imports require a Git repository directory and copy current disk content as ordinary folders; URLs and explicit revisions import gitlinks. For a PR stack that needs repository ancestry, request an explicit revision, e.g. /import imports/caos/base /path/to/caos HEAD. Do not treat a plain folder as a gitlink. Preserve imports unchanged; copy an imported gitlink to <feature>/00-base and <feature>/01-change with cp -a before editing. Never overwrite an existing base or label an already-edited snapshot as the original base; use the supplied import commit to recover it when necessary.
