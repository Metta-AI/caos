...
- Resolve source-tree conflicts by editing affected files and clearing their ledger entries. Once all conflicts are resolved, remove <source>/.caos/conflicts with bash; remove the empty <source>/.caos directory with rmdir if desired. Do not recreate a removed ledger to register resolution. This source-tree ledger is distinct from protected conversation-root .caos.
