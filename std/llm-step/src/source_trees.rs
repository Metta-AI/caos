...
- For reviewable changes, import a starting commit at imports/<repo>/base and preserve that import. Copy its gitlink to <feature>/00-work and write the imported commit ID to <feature>/00.base. A stack contains consecutive numbered gitlinks (00-name, 01-name, ...) and matching 00.base, 01.base files. Each base records where that layer was built; it stays unchanged as edits advance the gitlink. Ordinary notes can also live in the feature directory. No layer name means sealed or ready to publish.
- Bash needs conversation-relative paths declared for existing content. Use mv and cp -a so commit identity survives. For example, with paths ["imports/caos"]: mkdir -p feature; cp -a imports/caos/base feature/00-work. Write the imported commit hash to feature/00.base with write. To reference a known commit from CAOS, use caos get-hash <full-commit> /cas/source, then ln -s /cas/source <destination>; the next tool call exposes that gitlink as a directory. Host paths belong to the client; HTTPS imports use import_source.
- Each accepted code edit records a child commit automatically. Use the repository git-add-layer writer to promote final work into a named layer with an intentional message. It squashes tool-call history and starts the next work gitlink at that new commit. The repository git-rebase-i writer can rewrite several layers: pick= accepts a commit or A..B net-change range, squash= folds changes into the preceding commit, amend= lines supply a message, and branch= records layer boundaries. Read tool_help at its repository path, write <feature>/rebase/plan, and invoke it with run_tool.
...
- The git-rebase-i repository writer pauses at <feature>/rebase/work on conflicts. Resolve that draft with ordinary file tools, then invoke the same repository tool with stack=<feature> and action="continue". The native conflict report is <feature>/rebase/conflicts, outside the source tree. You can edit only the unfinished plan suffix; replacing the paused instruction discards its draft. Delete <feature>/rebase to abandon it. Completion replaces numbered layers, preserves other feature files, and fails safely if original layers changed.
...
- Preparing, delegating, merging, testing, and organizing a stack requires no publishing destination. Preserve imported snapshots and recorded layer bases. Finish the requested work without asking for repository publication settings.
