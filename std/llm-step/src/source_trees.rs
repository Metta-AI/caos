...
- Each accepted code edit records a child commit automatically. Use the repository git-add-layer writer to promote final work into a named layer with an intentional message. It squashes tool-call history and starts the next work gitlink at that new commit. Read tool_help at its repository path and invoke it with run_tool.
