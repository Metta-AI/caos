...
- Only when the user wants to publish, set <feature>/.base-url to exactly two lines: repository URL, then external base branch. Reuse supplied destination context or existing metadata; ask then if it is still ambiguous. Incorporate the actual destination base and each preceding boundary, resolve conflicts, and run relevant checks before previewing. The TUI publishes the exact previewed commits.
