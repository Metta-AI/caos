...
pub(super) fn execute(state: &mut progress::State, site: &CallSite<'_>) -> Result<(), String> {
    ...
    let provenance = match pinned(&state.conversation()?, site)? {
        Some(value) => value,
        None => {
            let token = if is_github_remote(source) && Path::new("/secret/github-token").exists() {
                Some(
                    fs::read_to_string("/secret/github-token")
...
fn resolve(
    ...
    let revision = git_locator::import::revision(revision)?;
