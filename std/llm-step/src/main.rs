...
mod tests {
    ...
    fn publication_pin_keeps_commit_and_lease_across_source_edits_and_restart() {
        ...
        for rejected in [false, true] {
            ...
            let (saved, _) = publish_source::pin(&mut state, &site, &pending)
                .unwrap()
                .unwrap();
            ...
            let (recovered, fresh) = publish_source::pin(&mut state, &site, &competing)
                .unwrap()
                .unwrap();
            assert!(!fresh);
            ...
            let outcome = if rejected {
                ...
                conversation_protocol::v3::publication::Outcome::from_observation(
                    &pending,
                    Some(commit.clone()),
                    "lost push acknowledgement".into(),
                    false,
                )
            };
