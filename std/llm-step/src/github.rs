...
pub(super) fn result(record: &CallRecord) -> Result<(Value, Option<Oid>), String> {
    ...
    let text = if value["status"] == "uncertain" {
        format!("UNCERTAIN: the command may have changed GitHub. Inspect with a new read call before taking further action; do not resend the write.\n{text}")
    } else {
        text
    };
