...
#
# Worker machine RAM is NOT a stack knob: each worker image declares its own via
# the `caos.fly.memory-mb` OCI config label (caosd reads it when provisioning the
# machine), so the size rides with the image and is identical on every stack. The
# rustc builder sets 2048; unlabelled workers default to 256.
#
